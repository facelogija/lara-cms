...
<head>
    ...
    <title>Pamokos redagavimas</title>
    <!-- Font Awesome -->
...
<main class="pt-5 mx-lg-5">
    <h3 class="mt-5 ml-3">Redagavimas/{{ $lessonq->title }}</h3>
...
<div class="card">
    ...
    <div class="card-body px-lg-5 pt-0">
        ...
        <!-- Form -->
        <form class="text-center" style="color: #757575;" action="/admin/conmanager/lesson/{{ $lessonq->id }}" enctype="multipart/form-data" method="post" >
            @csrf
            @method('PATCH')
            <div class="form-row">
                <div class="col">
                    <!-- Title -->
                    <div class="md-form">
                        <input type="text" id="title" name="title" class="form-control" value="{{ old('title') ?? $lessonq->title }}">
                    </div>
                </div>
                <div class="col">
                    <!-- Author -->
                    <div class="md-form">
                        <input type="text" id="author" name="author" class="form-control" value="{{ old('author') ?? $lessonq->author }}">
                    </div>
                </div>
            </div>
            <!-- Type/Category -->
            <select id="type" name="type"  class="mdb-select without-border">
                <option value="{{ old('type') ?? $lessonq->type }}">{{ old('type') ?? $lessonq->type }}</option>
            ...
            <!-- Video url  -->
            <div class="md-form">
                <input value="{{ old('video_url') ?? $lessonq->video_url }}" type="text" id="video_url" name="video_url" class="form-control" aria-describedby="video_url">
            ...
            <div class="md-form">
            <textarea class="form-control" id="instruction"  name="instruction">{{ old('instruction') ?? $lessonq->instruction }}</textarea>    
